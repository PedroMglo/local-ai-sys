...
**Resolução (2026-05-10):** `sync_repos()` agora usa `concurrent.futures.ThreadPoolExecutor` para processar repos em paralelo com submissão iterativa — cada repo é submetido individualmente com `should_throttle()` entre submissões, em vez de lançar todos de uma vez. `_wait_for_resources()` verifica recursos nas transições de fase (notas→repos, local→graphify). Nova configuração `[pipeline] max_workers = 4` em `rag.toml`. Novo dataclass `PipelineConfig` em `config.py`.
...
> **Fase 10 concluída em 2026-05-10.** Correcção do sistema de proteção de recursos que falhava durante `rag sync --all`, causando freeze da máquina. O sistema agora tem 3 camadas de proteção: auto-tune no arranque (ajusta batch sizes/workers ao hardware), verificação de recursos nas transições de fase (`_wait_for_resources()` entre notas→repos e local→graphify), e throttle contínuo (`should_throttle()` entre cada embedding batch e cada repo submission). `BATCH_SIZE=50` hardcoded removido de `chroma.py`. `subprocess.run()` no graphify tem timeout configurável (`graph_timeout=600`). Ficheiros afetados: `config.py`, `tuning.py`, `chroma.py`, `sync.py`, `builder.py`, `rag.toml`.
