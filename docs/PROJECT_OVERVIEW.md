...
> **Versão:** 0.5.4
> **Última atualização:** 2026-05-12 (prompts EN, multi-turn context, router heuristic improvements)
...
├── tests/                      # Testes automatizados (pytest) — 389 testes passam (3 skipped)
│   ├── __init__.py
...
**389 testes passam** (1 falha pré-existente por Qdrant version mismatch, 3 skipped) em 23 ficheiros, sem dependências externas (Ollama):
...
| Limitação                         | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cobertura de testes parcial**   | 389 testes passam (1 falha pré-existente por Qdrant version mismatch, 3 skipped, unit + integration + concorrência) cobrindo chunking (markdown + code + tree-sitter), router, budget, API auth, CLI, segurança, performance, adaptive top_k, vault_sync, cross-platform, manifest, ingest pipeline, ResourceGovernor, VectorStore protocol (incl. health), Dask engine, graphify incremental, concorrência (parallel queries, query during upsert, multi-collection, health under load) e multi-vault. Faltam e2e tests com Ollama |
...
| Aspeto                     | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Versão**                 | 0.5.4 (prompts EN, multi-turn context, router heuristic improvements, 2026-05-12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Router**                 | LLM (gemma3:4b) + keyword heuristic fallback — funcional. Multi-turn context awareness via `history` parameter. Heurística expandida com keywords PT+EN. Prompts do router em inglês                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **Testes**                 | 389 testes passam (1 falha pré-existente por Qdrant version mismatch, 3 skipped) em 23 ficheiros com pytest. Todos passam sem deps externas. Cobertura: 61%. Inclui testes de concorrência (`test_concurrency.py`) e `TestHealth` em vector store. CI inclui job `test-server-mode` com Qdrant service container                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| **Alterações 2026-05-12b** | **(1) Prompts model-internal convertidos para inglês** — `ROUTER_SYSTEM`, `REWRITE_SYSTEM`, `RAG_CONTEXT_INSTRUCTION`, `GRAPH_CONTEXT_INSTRUCTION`, `COMBINED_CONTEXT_INSTRUCTION`, `FALLBACK_WEAK_CONTEXT` agora em EN para melhor instruction-following. `SYSTEM_GENERAL` mantido em PT-PT (user-facing). **(2) Multi-turn context awareness** — `route_query()`, `_llm_route()`, `detect_intent_full()`, `build_rag_context()` e `/chat` endpoint aceitam `history` para detecção de follow-ups. LLM router inclui até 2 mensagens recentes como contexto. **(3) Router heuristic expandida** — novas keywords PT+EN (pipeline, codebase, workspace, modelfile, installed, configured, alias, functions) e padrões multi-palavra. Reason strings em inglês. **(4) Context block labels em EN** — `[SEMANTIC — PERSONAL NOTES]`, `[SEMANTIC — CODE: repo]`. **(5) Config** — `coder-pt` RAG enabled (`true`), `token_budget` 4000→6000. **Testes:** 389 passed, 1 pre-existing failure (Qdrant version mismatch), 3 skipped                                                                                                                                                                                                                                                                            |
