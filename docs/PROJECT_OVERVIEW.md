...
> **Versão:** 0.5.2
> **Última atualização:** 2026-05-11 (remoção ChromaDB, Qdrant como único backend)
...
├── tests/                      # Testes automatizados (pytest) — 416 testes (3 skipped)
│   ├── __init__.py
...
│   ├── test_vector_store.py    # VectorStore protocol: Qdrant embedded, factory, collection isolation
│   └── test_vault_sync.py
├── .github/
...
│       └── qdrant_store.py     # QdrantVectorStore — Qdrant (embedded/server) behind VectorStore protocol; meta.json recovery automático
└── source/
...
| Módulo            | Função                                                                                                                                                                                              |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base.py`         | `VectorStore` Protocol (`@runtime_checkable`) com 5 métodos + `QueryResult` dataclass + `create_store()` factory (Qdrant-only desde v0.5.2). `chroma_store.py` e `chromadb` completamente removidos |
| `qdrant_store.py` | `QdrantVectorStore` — Qdrant (embedded ou server mode), 1024d cosine, SHA256 string→uint64 ID mapping, `meta.json` recovery automático. Único backend suportado                                     |

### Retrieval (`obsidian_rag/retrieval/`)

| Módulo             | Função                                                                                                                                                                                                                              |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rag.py`           | `build_rag_context()` — entry point. Usa `VectorStore` protocol via `_get_store()` singleton. Adaptive top_k (`_estimate_complexity`). Multi-estratégia: vector + keyword + title. Deduplicação, threshold dinâmico, relevance gate |
...
**Qdrant server mode (opcional):** O `docker-compose.yml` inclui um serviço `qdrant` (qdrant/qdrant:v1.13.2) nas portas 6333/6334, ativado via profile: `docker compose --profile qdrant up`. Para usar Qdrant em server mode (recomendado para >50k chunks ou acesso concorrente), configurar `qdrant_url = "http://localhost:6333"` em `rag.toml`. Por defeito, o Qdrant corre em modo embedded (sem servidor externo). Ver [docs/QDRANT_SERVER_MODE.md](QDRANT_SERVER_MODE.md) para o guia completo de migração, incluindo setup Docker, configuração, migração de dados, verificação, rollback e notas de segurança.
...
| `[store]`          | `backend="qdrant"` (único backend suportado), `qdrant_url`, `qdrant_api_key`. Factory `create_store()` lê esta secção. Controla o backend para **leituras e escritas** (pipeline sync, ingest, backup). ChromaDB removido em v0.5.2                                                                                                                                                                       |
...
**416 testes** (3 skipped) em 21 ficheiros, sem dependências externas (Ollama):
...
| Limitação                         | Descrição                                                                                                                                                                                                                                                                                                                           |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cobertura de testes parcial**   | 416 testes (3 skipped, unit + integration) cobrindo chunking (markdown + code + tree-sitter), router, budget, API auth, CLI, segurança, performance, adaptive top_k, vault_sync, cross-platform, manifest, ingest pipeline, ResourceGovernor, VectorStore protocol, Dask engine e graphify incremental. Faltam e2e tests com Ollama |
...
| Aspeto                  | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Versão**              | 0.5.2 (graphify incremental 2026-05-11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| **Documentação**        | Este ficheiro + `IMPROVEMENTS_AND_RISKS.md` + `QDRANT_SERVER_MODE.md` (guia migração embedded→server) + roadmap v1.1 (Fase 20 — 13 tarefas de escalabilidade, #186 e #187 concluídos)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Testes**              | 416 testes (3 skipped) em 21 ficheiros com pytest. Todos passam sem deps externas. Cobertura: 61%. Inclui 18 novos testes para graphify incremental (2026-05-11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **CI/CD**               | 4 workflows GitHub Actions: `ci.yml` (matrix 3 OS × 2 Python + lint + security), `docker.yml`, `release.yml`, `codeql.yml`. Sem Ollama/GPU em CI. **Pre-commit hooks** (`.pre-commit-config.yaml`) espelham o CI localmente: `ruff` lint (pre-commit, só `obsidian_rag/`), `mypy` type-check (pre-commit), `pytest` com coverage gate ≥30% (pre-push)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
