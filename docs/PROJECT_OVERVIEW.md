...
> **Versão:** 0.5.5
> **Última atualização:** 2026-05-12 (System Context: live machine state routing, SYSTEM/SYSTEM_AND_RAG modes, `system_context.py`)
...
├── rag.internal.toml           # Defaults técnicos (não editar — gerido pelo projecto)
├── rag.user.toml               # Configuração do utilizador (vault, modelos, API — editável)
...
│   ├── QDRANT_SERVER_MODE.md   # Guia de migração Qdrant embedded → server mode
│   └── POSTMORTEM_SYNC_FREEZE.md # Postmortem do freeze de sync (histórico)
├── tests/                      # Testes automatizados (pytest) — 443 testes passam
│   ├── __init__.py
...
│   ├── test_governor.py        # ResourceGovernor: thresholds, lifecycle, wait, metrics, PSI, VRAM, watchdog, tuning compat (43 testes)
│   ├── test_integration.py     # Integration tests: endpoints + Qdrant in-memory
│   ├── test_low_priority.py    # Thread safety, Unicode, __all__, reranker cache, stop words bilíngues
│   ├── test_vector_store.py    # VectorStore protocol: Qdrant embedded/server, factory, collection isolation, health()
│   ├── test_concurrency.py     # Concorrência: parallel queries, query during upsert, multi-collection upsert, health under load
│   └── test_vault_sync.py
├── .github/
...
├── data/
│   ├── qdrant/                 # Qdrant vector store persistente (embedded mode)
...
│   │   ├── init_cmd.py         # `rag init` — wizard interactivo, gera rag.user.toml
...
│   │   ├── router.py           # LLM router + heurística keyword (incl. SYSTEM/SYSTEM_AND_RAG routing)
│   │   ├── intent.py           # Mapeamento ContextMode → QueryIntent (incl. use_system)
│   │   ├── system_context.py   # Colector de estado live do sistema (free, nvidia-smi, df, uptime, ps, ip, sensors)
...
│   └── store/
│       ├── __init__.py         # Singleton process-wide `get_store()` com threading.Lock + `_reset_store()` para testes
│       ├── base.py             # VectorStore Protocol + QueryResult + create_store() factory (Qdrant-only)
│       └── qdrant_store.py     # QdrantVectorStore — Qdrant (embedded/server) behind VectorStore protocol; _retry() com exponential backoff; health() method; meta.json recovery automático
└── source/
...
rag sync -l:
  ...
  3. **Notas e repos processados via Bounded Ingest Pipeline** (v1.1):
     a. Scanner thread — iter_note_files() (notas) / iter_repo_files() (repos) descobre ficheiros alterados
     ...
     b. Parser pool — via create_parser_pool() factory:
        ...
        Notas: chunk_note() | Código: chunk_file()
     ...
     d. Writer thread — upsert na coleção correspondente com embeddings pré-calculados
        Notas: "obsidian_vault" | Código: "code_repos" (configurável)
     ...
     f. ResourceGovernor (Phase 2) — daemon thread monitora psutil + PSI + VRAM cada 1s;
        check() retorna GovernorAction (CONTINUE|THROTTLE|REDUCE|PAUSE|ABORT) sem blocking;
        PSI (Pressure Stall Information) deteta stalls antes de RAM% atingir thresholds;
        VRAM via nvidia-ml-py (opcional) para monitorizar GPU;
        governor partilhado entre sync_repos() e IngestPipeline
     g. Global timeout watchdog — aborta pipeline após max_run_seconds (default 30min)
     h. Worker memory limits — resource.setrlimit(RLIMIT_AS) em cada worker (25% da RAM total)
     i. Stale cleanup: remove chunks de ficheiros apagados via manifest
     j. gc.collect() após conclusão do pipeline

rag sync -g:
  1. **Detecção incremental 3-tier** por repo (build_graph()):
     a. `_detect_changes(repo_path, manifest_path)` lê `manifest.json` do graphify
        (formato: file_path → {mtime, hash}), compara MD5 hashes contra ficheiros actuais
     b. Ficheiros novos (ausentes do manifest) também detectados
     c. `_DOC_EXTENSIONS = {.md, .txt, .rst, .adoc}` — extensões que requerem LLM
     d. Resultado: `(has_changes, has_doc_changes)`
  2. Decisão por repo:
     ├── Sem alterações → **skip** (sem subprocess, sem graphify call)
     ├── Só código alterado (.py, .js, etc.) → `graphify update` (AST-only, sem LLM, rápido)
     └── Docs alterados (.md, .txt, etc.) → `graphify extract` (AST + LLM semântico)
  3. `--force` → bypassa detecção, executa sempre `graphify extract` completo
  4. Timeout configurável: graph_timeout=900s
  5. Throttle check antes de cada repo em build_graphs()
  6. Gera graph.json (node-link), GRAPH_REPORT.md, community_summaries.json
  7. export_all() — exporta grafos como notas Markdown para ~/Obsidian/knowledge-graphs/
  8. Invalida cache do grafo em memória
...
rag sync --all:
  ...
  Proteção de recursos em 6 camadas:
    ...
    3. ResourceGovernor contínuo — daemon thread no pipeline (check() + wait_until_safe()) com PSI + VRAM
    4. Worker memory limits — resource.setrlimit(RLIMIT_AS) limita cada worker a 25% da RAM total
    5. Global timeout watchdog — daemon thread aborta pipeline após max_run_seconds (default 30min)
    6. cgroup externo (opcional) — scripts/rag-cgroup.sh aplica MemoryMax e CPUQuota via systemd
...
3. detect_intent_full() — router decide o modo (com `history` para follow-ups):
   ...
   ├── RAG_AND_GRAPH → ambos
   ├── SYSTEM → dados live do sistema (sem Qdrant)
   ├── SYSTEM_AND_RAG → dados live + pesquisa RAG
   └── CLARIFY → pede esclarecimento ao utilizador
4. Router: 2 camadas — LLM (gemma3:4b, 15s timeout) → keyword heuristic fallback
   LLM router inclui até 2 mensagens anteriores do utilizador como contexto para detecção de follow-ups
5. Adaptive top_k:
...
11. Token budget allocation: 6000 tokens, split 40/40/20 (notas/código/grafo)
...
| Módulo            | Função                                                                                                                                                                                                                                                              |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `base.py`         | `VectorStore` Protocol (`@runtime_checkable`) com 6 métodos (incl. `health()`) + `QueryResult` dataclass + `create_store()` factory (Qdrant-only desde v0.5.2). `chroma_store.py` e `chromadb` completamente removidos                                              |
| `qdrant_store.py` | `QdrantVectorStore` — Qdrant (embedded ou server mode), 1024d cosine, SHA256 string→uint64 ID mapping, `meta.json` recovery automático, `_retry()` com exponential backoff (3 retries, 0.5s base) em todos os API calls, `health()` method. Único backend suportado |
| `__init__.py`     | Singleton process-wide `get_store()` com `threading.Lock` — retrieval (`rag.py`) e pipeline (`sync.py`) partilham a mesma instância. `_reset_store()` para testes                                                                                                   |

### Retrieval (`obsidian_rag/retrieval/`)

| Módulo             | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rag.py`           | `build_rag_context()` — entry point. Aceita parâmetro `history` para multi-turn context awareness. Usa `VectorStore` protocol via `_get_store()` (thin proxy para `store.get_store()` singleton process-wide). Adaptive top_k (`_estimate_complexity`). Multi-estratégia: vector + keyword + title. Deduplicação, threshold dinâmico, relevance gate. Labels de contexto em inglês: `[SEMANTIC — PERSONAL NOTES]`, `[SEMANTIC — CODE: repo]`. **System context:** para modo SYSTEM, retorna dados live sem Qdrant; para SYSTEM_AND_RAG, combina dados live com resultados RAG. `"system"` registado em `sources_used`. Trace rejection reasons em inglês |
| `router.py`        | `ContextMode` enum (NO_CONTEXT, RAG_ONLY, GRAPH_ONLY, RAG_AND_GRAPH, **SYSTEM**, **SYSTEM_AND_RAG**, CLARIFY). LLM router (gemma3:4b) com keyword heuristic fallback. `route_query()` e `_llm_route()` aceitam `history` para detecção de follow-ups multi-turn. Heurística expandida: keywords PT+EN (pipeline, codebase, workspace, modelfile, installed, configured, alias, functions) e padrões multi-palavra ("este projeto", "my pipeline", etc.). **System routing:** `_SYSTEM_SIGNALS` (ram, gpu, cpu, disco, temperatura, nvidia, etc.), `_SYSTEM_PATTERNS` ("quanto de ram", "disk space", etc.), `_SYSTEM_FALSE_POSITIVES` ("machine learning", "system design", etc.) para supressão de falsos positivos. Reason strings em inglês |
| `intent.py`        | Mapeia `ContextMode` → `QueryIntent(use_notes, use_code, use_graph)`. `detect_intent_full()` aceita `history` para multi-turn context awareness                                                                                                                                                                                                                                                                                                                                 |
...
| Módulo               | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `builder.py`         | Wrapper para `graphify extract`/`graphify update` via `subprocess.run` com timeout configurável (`graph_timeout`). **Modo incremental 3-tier:** (1) sem alterações → skip subprocess, (2) só código alterado → `graphify update` (AST-only, sem LLM), (3) docs alterados → `graphify extract` (AST + LLM). `_detect_changes()` lê `manifest.json` do graphify (path→{mtime,hash}) e compara MD5 hashes. `_DOC_EXTENSIONS` = `.md`, `.txt`, `.rst`, `.adoc`. `force=True` bypassa detecção. Throttle check antes de cada repo em `build_graphs()`. Injeta `OLLAMA_BASE_URL` e `OLLAMA_API_KEY=ollama`. Trata `TimeoutExpired` graciosamente |
...
| Módulo        | Função                                                                                                                                                                                                                                                                                                                                          |
| ------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `app.py`      | FastAPI com `lifespan` (preload VectorStore, httpx pool). Middleware de autenticação via API key (`Bearer`). `serve()` recusa `0.0.0.0` sem `api_key`. Endpoints REST + `/chat`. Usa `_get_store()` / `_query_store()` via `VectorStore` protocol. `/chat` endpoint passa mensagens anteriores ao router para detecção de follow-ups multi-turn |
| `schemas.py`  | Modelos Pydantic: QueryRequest, ChatRequest, ChunkResult, RepoInfo, StatsResponse (`data_path` em vez do antigo `chroma_path`), etc.                                                                                                                                                                                                            |
...
| `init_cmd.py`     | `rag init` — wizard interactivo: gera `rag.user.toml`, detecta vault/Git (cross-platform), lista modelos Ollama, valida paths perigosos (Windows/macOS/Linux)              |
...
| `migrate_cmd.py`  | `rag migrate --from X --to Y --collections ...` — migração entre backends de vector store com re-embedding. Contém referência histórica a ChromaDB para migrações legacy   |

### Pipeline (`obsidian_rag/pipeline/`)

| Módulo           | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sync.py`        | Quatro modos: `-l` (notas + repos), `-g` (graphify), `--all`, **`--system`** (snapshot estático do sistema). **Notas e repos processados via bounded ingest pipeline** (`IngestPipeline`) — 4 estágios conectados por queues bounded com backpressure. Notas usam `IngestSource(source_type="vault")` com `collection_name="obsidian_vault"`; repos usam `source_type="code"` com `collection_name` configurável. `IngestManifest` fornece skip incremental (mtime/size/SHA256) para ambos. `os.nice(10)` no arranque para não bloquear o desktop/OS. Proteção de recursos em 4 camadas: auto-tune no arranque, ResourceGovernor nas verificações entre fases (`_wait_for_resources()`), ResourceGovernor contínuo no pipeline (partilhado com IngestPipeline), e cgroup externo opcional (`rag-cgroup.sh`). `KeyboardInterrupt` tratado graciosamente — imprime mensagem de aviso e sai com código 130. Usa `get_store()` singleton (partilhado com retrieval layer) para todas as operações — elimina instâncias QdrantClient duplicadas. `_sync_chunks_to_store()` e `chunk_all_notes()` deprecados (v1.1)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `governor.py`    | `ResourceGovernor` — daemon thread que amostra `psutil` a cada 1s (intervalo configurável). `check()` retorna `GovernorAction` (CONTINUE/THROTTLE/REDUCE/PAUSE/ABORT) sem blocking (lê snapshot em cache). `wait_until_safe(timeout)` bloqueia até recursos libertarem. `ResourceSnapshot` dataclass com ram_percent, ram_available_gb, cpu_percent, disk_free_gb, swap_percent, swap_used_gb, psi_memory_full_avg10, psi_io_full_avg10, psi_cpu_some_avg10, vram_used_gb, vram_total_gb, vram_percent. **PSI (Pressure Stall Information):** lê `/proc/pressure/{memory,io,cpu}` para detetar stalls antes de RAM% atingir thresholds — `memory full avg10 > 25%` → PAUSE, `> 10%` → REDUCE, `> 5%` → THROTTLE; `io full avg10 > 40%` → PAUSE, `> 20%` → REDUCE, `> 10%` → THROTTLE; `cpu some avg10 > 50%` → THROTTLE. **VRAM:** `_read_vram()` via `nvidia-ml-py` (opcional, dep `gpu`) lê VRAM usada/total/% da GPU 0. **Swap:** 3 thresholds (max/pause/abort) + swap storm detection (delta > 0.5 GB). Três thresholds RAM: `max_memory_percent` → REDUCE, `pause_memory_percent` → PAUSE, `abort_memory_percent` → ABORT. Disco <1GB → ABORT. CPU > max+10% → THROTTLE. Ficheiro JSONL de métricas com campos PSI e VRAM. Thread-safe via `threading.Lock`. Helpers: `_parse_psi_line()`, `_read_psi()`, `_read_vram()`                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `ingest.py`      | `IngestPipeline` — pipeline paralelo bounded com 4 estágios + watchdog: (1) scanner thread descobre ficheiros alterados via `iter_repo_files()`/`iter_note_files()`, (2) parser pool via `create_parser_pool()` factory (ProcessPoolExecutor ou DaskParserPool conforme `engine` config) parse ficheiros em chunks com isolamento de memória, (3) embedding batcher acumula micro-batches (count ≤24, chars ≤48k, ou timeout ≥1s) e chama `embed_texts()` com governor.check()/wait_until_safe(), (4) writer thread upserts via `VectorStore.upsert_batch()` com embeddings pré-calculados. **Global timeout watchdog:** daemon thread que aborta o pipeline após `max_run_seconds` (default 1800s/30min) para evitar que o processo principal fique preso indefinidamente. **Worker memory limits:** `_apply_worker_rlimits()` chama `resource.setrlimit(RLIMIT_AS, total_ram // 4)` em cada worker do ProcessPoolExecutor — se um parser consumir >25% da RAM total, recebe `MemoryError` em vez de causar OOM. `_parse_file_worker()` aplica rlimits na primeira invocação. Construtor aceita `store` (VectorStore) + `collection_name` + `pipeline_config` opcional (para selecção de engine) + `max_run_seconds`. Aceita `governor` opcional — cria um automaticamente se não fornecido. **Stale cleanup global** `_cleanup_stale_global(all_manifest_ids)`: recolhe union de IDs de todos os repos antes de eliminar — evita deleção cruzada de chunks entre repos. `_cleanup_stale(source)` preservado para retrocompatibilidade com testes. Backpressure via `Queue(maxsize=...)`. **Logs de progresso** `[scan]`/`[parse]`/`[embed]`/`[write]` por etapa. Parse errors registados com `{e!r}` + traceback completo. `IngestResult` sumariza métricas |
...
| `backup.py`      | Backup timestamped do vector store (`shutil.copytree`) com rotação (mantém últimas 3 cópias). Função `backup_store()`. Ficheiros de backup nomeados `store_backup_*`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Módulo         | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `templates.py` | **Política de língua dual:** `SYSTEM_GENERAL` (PT-PT, user-facing) mantido em português. Todos os prompts model-internal convertidos para **inglês** para melhor instruction-following dos modelos Ollama: `ROUTER_SYSTEM` + `ROUTER_USER_TEMPLATE` (6 exemplos EN), `REWRITE_SYSTEM` (inclui descrição do domínio da KB), `RAG_CONTEXT_INSTRUCTION` (anti-hallucination, context-over-general-knowledge), `GRAPH_CONTEXT_INSTRUCTION` (direcção explícita, impact chains), `COMBINED_CONTEXT_INSTRUCTION` (labels `[SEMANTIC]`/`[STRUCTURAL]`), `FALLBACK_WEAK_CONTEXT` (`[SYSTEM NOTE]` anti-echo guard), **`SYSTEM_CONTEXT_INSTRUCTION`** (instruções para interpretar dados live do sistema). Helper `get_context_instruction(sources_used)` seleciona o prompt correcto — inclui handling de `"system"` e fontes combinadas |
...
| `deepseek-r1:8b` / `deepseek-r1-pt` | Chain-of-thought reasoning                       | ~45 tok/s  | 100% GPU |
| `qwen2.5-coder:7b` / `coder-pt`     | Extração graphify + chat código (RAG desativado) | ~43 tok/s  | 100% GPU |
...
### Optimização GPU (NVIDIA)

O auto-tune (`tuning.py`) detecta VRAM real via `pynvml` e ajusta parâmetros automaticamente quando VRAM ≥ 6GB:

| Parâmetro                   | Sem GPU / <6GB VRAM | Com GPU ≥ 6GB VRAM |
| --------------------------- | ------------------- | ------------------ |
| `embedding_batch_size`      | 15–30 (por RAM)     | 50                 |
| `embedding_batch_max_chars` | 48000               | 60000              |
| `graph_parallel_jobs`       | 1 (sequencial)      | 2 (paralelo)       |

**Env vars Ollama recomendadas** (adicionar a `~/.bashrc`, `systemd service`, ou `/etc/environment`):

```bash
# Permite requests concorrentes (embedding + graphify em simultâneo)
OLLAMA_NUM_PARALLEL=2
# Mantém bge-m3 + modelo graphify carregados em VRAM simultaneamente
OLLAMA_MAX_LOADED_MODELS=2
```

**Budget VRAM (RTX 4060 8GB):** `qwen2.5-coder:7b` ~4.5GB + `bge-m3` ~0.5GB = ~5GB → cabe com margem de ~3GB para overhead do OS/Ollama.

## APIs e endpoints

**Base URL:** `http://127.0.0.1:8484`

> **Autenticação:** Quando `api_key` está configurado em `rag.user.toml`, todos os endpoints (exceto `/health`, `/docs`, `/openapi.json`, `/redoc`) requerem header `Authorization: Bearer <key>`. Quando vazio (defeito), a auth é desativada para retrocompatibilidade.
...
_\*Apenas quando `api_key` está definido em `rag.user.toml` ou via `RAG_API_API_KEY`._
...
| Comando                                                  | Descrição                                                                                                                                                     |
| -------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rag init`                                               | Wizard interactivo: gera `rag.user.toml`, detecta vault Obsidian e repos Git, lista modelos Ollama, valida paths perigosos, força `api_key` se bind `0.0.0.0` |
...
| `rag sync -l [--vault NAME]`                             | Sincroniza embeddings de notas (sequencial) + repos (bounded pipeline paralelo com backpressure). `--vault` filtra por vault                                  |
| `rag sync -g [--force]`                                  | Constrói knowledge graphs via Graphify                                                                                                                        |
| `rag sync --all [--vault NAME]`                          | Executa `-l` seguido de `-g` (também executável via `scripts/rag-cgroup.sh` com limites de cgroup)                                                            |
| `rag sync --system`                                      | Gera snapshot estático do sistema em `source/AI-context/system-profile/live-state.md` para indexação RAG                                                      |
| `rag serve`                                              | Inicia FastAPI em `127.0.0.1:8484` (recusa `0.0.0.0` sem `api_key`)                                                                                           |
| `rag query "texto" [-n N] [--vault NAME]`                | Pesquisa semântica one-shot (stdout ou `--json`). `--vault` filtra por vault                                                                                  |
...
| `rag migrate --from X --to Y [--collections ...]`        | Migrar dados entre backends de vector store com re-embedding (comando mantido para migrações legacy Chroma → Qdrant)                                          |
...
| ------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qdrant-client`     | ≥ 1.9    | Cliente Qdrant — único vector store backend. Modo embedded (default) ou server via `qdrant_url`                                                                  |
...
| `nvidia-ml-py`      | ≥ 12.0   | Monitorização VRAM — **dependência opcional** (`pip install obsidian-rag[gpu]`). Usado pelo `ResourceGovernor` para ler VRAM da GPU via NVML                     |
| `memray`            | ≥ 1.13   | Memory profiler — **dependência opcional** (`pip install obsidian-rag[profile]`). Para diagnóstico de memory leaks em native extensions                          |
...
> **Nota (v0.4.0):** `graphifyy` foi promovido de dependência opcional a obrigatória. É um pacote pure Python leve, instalado sempre, mas a execução de grafos continua opt-in via `[graphify] enabled` em `rag.user.toml`.
...
| `pre-commit`     | ≥ 3.2  | Git hooks locais (lint, type-check, testes) |
...
| `tomllib`                    | Leitura dos ficheiros TOML de config (Python 3.11+)                                          |
...
| `resource`                   | `setrlimit(RLIMIT_AS)` em workers do parser pool — limita memória por worker a 25% da RAM    |
...
| **Qdrant**       | Único vector store (embedded/server, 1024d cosine) — ChromaDB removido em v0.5.2 |
...
- Vault Obsidian em `~/Obsidian/Vault` (configurável em `rag.user.toml`)
...
```bash
# Wizard interactivo — gera rag.user.toml
rag init
...
make qdrant     # docker compose --profile qdrant up -d (Qdrant server mode)
make qdrant-down  # docker compose --profile qdrant down
```

### Environment overrides

Qualquer opção de `rag.user.toml` (ou `rag.internal.toml`) pode ser sobreposta via variável de ambiente `RAG_{SECÇÃO}_{CHAVE}`:
...
**Qdrant server mode (opcional, recomendado para concorrência):** O `docker-compose.yml` inclui um serviço `qdrant` (qdrant/qdrant:v1.13.2) nas portas 6333/6334, ativado via profile: `docker compose --profile qdrant up` (ou `make qdrant`). O serviço tem healthcheck (`curl /healthz`, 15s interval), limites de memória (`mem_limit: 512m`, `mem_reservation: 256m`) e tuning mmap (`ON_DISK_PAYLOAD=true`, `MMAP_THRESHOLD_KB=20480`). O `rag.user.toml` configura `qdrant_url = "http://localhost:6333"` por defeito (server mode), eliminando deadlocks de file-lock que ocorriam em modo embedded sob acesso concorrente (múltiplos modelos AI a fazer queries RAG simultaneamente via orquestrador). Para rollback a embedded: `qdrant_url = ""`. Ver [docs/QDRANT_SERVER_MODE.md](QDRANT_SERVER_MODE.md) para o guia completo de migração.
...
A configuração é dividida em dois ficheiros TOML com merge automático:

| Ficheiro            | Propósito                                                          | Editar?                    |
| ------------------- | ------------------------------------------------------------------ | -------------------------- |
| `rag.user.toml`     | Configuração do utilizador: vault, modelos, API, repos, debug      | **Sim** — personalizações  |
| `rag.internal.toml` | Defaults técnicos: chunking, performance, context policy, pipeline | Não — gerido pelo projecto |

Prioridade (maior → menor): **env vars** > `rag.user.toml` > `rag.internal.toml` > defaults de código.

Compatibilidade: `rag.user.toml` + `rag.internal.toml` são os únicos ficheiros de config suportados.

### Secções configuráveis (`rag.user.toml`)

| Secção             | Opções-chave                                                                                                                                                                                                                                                                                                                                                                                              |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `[paths]`          | `source_dir`, `data_dir`, `vault_dir`, `vault_dirs` (multi-vault list, overrides `vault_dir`)                                                                                                                                                                                                                                                                                                             |
...
| `[retrieval]`      | `top_k=10`, `score_threshold=0.45`, `dynamic_threshold_ratio=0.75`, `token_budget=6000`, `context_mode=auto`                                                                                                                                                                                                                                                                                              |
| `[api]`            | `host=127.0.0.1`, `port=8484`, `api_key=""` (vazio = sem auth; override: `RAG_API_API_KEY`)                                                                                                                                                                                                                                                                                                               |
| `[models]`         | Per-model RAG toggle (ex: `"coder-pt" = true`)                                                                                                                                                                                                                                                                                                                                                            |
...
| `[store]`          | `backend="qdrant"` (único backend suportado), `qdrant_url="http://localhost:6333"` (server mode por defeito para concorrência), `qdrant_api_key`. Factory `create_store()` lê esta secção. Controla o backend para **leituras e escritas** (pipeline sync, ingest, backup). ChromaDB removido em v0.5.2. Para rollback a embedded: `qdrant_url = ""`                                                      |
...
# Instalar pre-commit hooks (mirror do CI pipeline)
...
| ------------------------ | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ci.yml`                 | push/PR → main           | lint, test matrix (3 OS × 2 Python), CLI smoke (3 OS), config tests, security audit, **test-server-mode** (Qdrant service container + concurrency tests) |
...
**443 testes passam** em 23 ficheiros, sem dependências externas (Ollama):
...
| `test_router.py`               | 19     | `_heuristic_route` com queries PT/EN, sinais de grafo/locais, edge cases, **system routing** (SYSTEM/SYSTEM_AND_RAG modes), **false-positive suppression** ("machine learning", "system design")                                                                                                                  |
...
| `test_init.py`                 | —      | `rag init`: validação de paths perigosos, geração de rag.user.toml                                                                                                                                                                                                                                                  |
...
| `test_adaptive_topk.py`        | 22     | `_estimate_complexity`, adaptive top_k scaling (simple/normal/complex), `_scale_k_by_size()` collection-size scaling, `_cached_count()` TTL cache                                                                                                                                                                   |
| `test_integration.py`          | 16     | `/query`, `/query/code`, `/stats` com Qdrant in-memory via `QdrantVectorStore` + `_get_store` mock, validação Pydantic (422)                                                                                                                                                                                        |
...
| `test_graphify_incremental.py` | 18     | `_file_md5` (hash, different content, missing file), `_detect_changes` (no manifest, corrupt manifest, no changes, code/doc changed, deleted code/doc, new code/doc, hidden dirs ignored, .txt as doc), `build_graph` incremental (skip no changes, update code-only, extract doc changed, force ignores detection) |
| `test_vector_store.py`         | 38     | VectorStore protocol: Qdrant (embedded). upsert, count, get_existing_ids, delete, query (incl. `filters=` metadata filtering), collection isolation, cosine similarity, factory `create_store()`                                                                                                                    |

Fixtures partilhadas em `conftest.py`: `tmp_source_dir`, `sample_markdown_note`, `navigation_note`, `sample_python_source`. Testes tree-sitter usam `pytest.importorskip("tree_sitter")` — skipped se tree-sitter não instalado.

## Limitações conhecidas

| Limitação                         | Descrição                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cobertura de testes parcial**   | 443 testes passam (unit + integration + concorrência) cobrindo chunking (markdown + code + tree-sitter), router (incl. system routing e false-positive suppression), budget, API auth, CLI, segurança, performance, adaptive top_k, vault_sync, cross-platform, manifest, ingest pipeline, ResourceGovernor, VectorStore protocol (incl. health), Dask engine, graphify incremental, concorrência (parallel queries, query during upsert, multi-collection, health under load) e multi-vault. Faltam e2e tests com Ollama |
...
| **Single-user**                   | A arquitetura não suporta múltiplos utilizadores concorrentes de forma otimizada. Qdrant server mode resolve concorrência de queries RAG por múltiplos modelos AI via orquestrador                                                                                                                                                                                                                                                                                                                                                  |
...
| Aspeto                     | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Versão**                 | 0.5.5 (System Context: live machine state routing, SYSTEM/SYSTEM_AND_RAG modes, 2026-05-12)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **Pipeline RAG**           | Completo e funcional: sync (4 backends) → chunking (Markdown headers + Python AST + tree-sitter multi-linguagem) → embeddings → Qdrant (único vector store) → retrieval multi-estratégia com adaptive top_k + query-time metadata filtering. **Notas e repos processados via bounded ingest pipeline** (v1.1): 4 estágios (scanner → parser pool → embedding batcher → writer) conectados por queues bounded com backpressure. `IngestManifest` SQLite fornece skip incremental (mtime/size/SHA256) para ambos. **Parser pool via factory `create_parser_pool()`** (Phase 5): `ProcessPoolExecutor` (default, `engine="local"`) ou `DaskParserPool` distribuído (`engine="dask"`, requer `pip install obsidian-rag[dask]`). `ResourceGovernor` (Phase 2) monitoriza recursos continuamente via daemon thread. `os.nice(10)` para não bloquear o desktop. Proteção de recursos ao nível do OS via `rag-cgroup.sh` (opcional). **VectorStore Protocol** (Phase 3): abstração `VectorStore` com factory `create_store()` (Qdrant-only desde v0.5.2). `query()` aceita `filters: dict` para metadata filtering (repo_name, symbol_type, etc.). ChromaDB completamente removido em v0.5.2 — `chroma_store.py` eliminado, `chromadb` removido das dependências, `create_store()` simplificado para Qdrant-only |
| **Knowledge graphs**       | Funcional com Graphify (agora dep obrigatória, opt-in na execução). **Modo incremental 3-tier** (#186): skip quando sem alterações, `graphify update` (AST-only) para código, `graphify extract` (AST+LLM) para docs. Enrichment + Mermaid + export Obsidian                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **Config**                 | `_LazySettings` proxy — config só carrega no primeiro acesso. `rag init` e `rag doctor` funcionam sem `rag.user.toml`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| **Router**                 | LLM (gemma3:4b) + keyword heuristic fallback — funcional. Multi-turn context awareness via `history` parameter. Heurística expandida com keywords PT+EN. **System routing:** modos SYSTEM e SYSTEM_AND_RAG para queries sobre hardware/recursos. False-positive suppression para termos compostos ("machine learning", "system design"). Prompts do router em inglês                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **Documentação**           | Este ficheiro + `IMPROVEMENTS_AND_RISKS.md` + `QDRANT_SERVER_MODE.md` (guia migração embedded→server) + roadmap v1.1 (Fase 20 — 13 tarefas de escalabilidade, #186, #187 e #188 concluídos)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| **Testes**                 | 443 testes passam em 23 ficheiros com pytest. Todos passam sem deps externas. Cobertura: 61%. Inclui testes de concorrência (`test_concurrency.py`), `TestHealth` em vector store, e **6 novos testes de system routing** (SYSTEM, SYSTEM_AND_RAG, false-positive suppression). CI inclui job `test-server-mode` com Qdrant service container                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **CI/CD**                  | 4 workflows GitHub Actions: `ci.yml` (matrix 3 OS × 2 Python + lint + security + **test-server-mode** com Qdrant service container), `docker.yml`, `release.yml`, `codeql.yml`. Sem Ollama/GPU em CI. **Pre-commit hooks** (`.pre-commit-config.yaml`) espelham o CI localmente: `ruff` lint (pre-commit, só `obsidian_rag/`), `mypy` type-check (pre-commit), `pytest` com coverage gate ≥30% (pre-push)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Hotfixes 2026-05-11**    | **3 bugs críticos corrigidos:** (1) `_cleanup_stale` em `ingest.py` apagava chunks de todos os repos além do actual — substituído por `_cleanup_stale_global` que recolhe union de IDs de todos os repos; (2) `_split_long_text` em `markdown.py` entrava em loop infinito em ficheiros longos (ex: `PROJECT_OVERVIEW.md`) causando `MemoryError` — corrigido com guard de avanço obrigatório do cursor; (3) `qdrant_store.py` crashava em startup com `JSONDecodeError` se `meta.json` ficasse corrompido (0 bytes) — adicionados `_recover_meta_if_corrupt()` e `_backup_meta()`. Vector store: `obsidian_vault=537 chunks`, `code_repos=150 chunks`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Alterações 2026-05-12b** | **(1) Prompts model-internal convertidos para inglês** — `ROUTER_SYSTEM`, `REWRITE_SYSTEM`, `RAG_CONTEXT_INSTRUCTION`, `GRAPH_CONTEXT_INSTRUCTION`, `COMBINED_CONTEXT_INSTRUCTION`, `FALLBACK_WEAK_CONTEXT` agora em EN para melhor instruction-following. `SYSTEM_GENERAL` mantido em PT-PT (user-facing). **(2) Multi-turn context awareness** — `route_query()`, `_llm_route()`, `detect_intent_full()`, `build_rag_context()` e `/chat` endpoint aceitam `history` para detecção de follow-ups. LLM router inclui até 2 mensagens recentes como contexto. **(3) Router heuristic expandida** — novas keywords PT+EN (pipeline, codebase, workspace, modelfile, installed, configured, alias, functions) e padrões multi-palavra. Reason strings em inglês. **(4) Context block labels em EN** — `[SEMANTIC — PERSONAL NOTES]`, `[SEMANTIC — CODE: repo]`. **(5) Config** — `coder-pt` RAG enabled (`true`), `token_budget` 4000→6000. **(6) System Context** — novo módulo `system_context.py` para recolha live de estado do sistema. Modos `SYSTEM` e `SYSTEM_AND_RAG` no router. `sync --system` para snapshot estático. 6 novos testes de system routing. **Testes:** 443 passed                                                                                                                                                                                                                                                                            |
