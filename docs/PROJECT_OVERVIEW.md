...
> **Versão:** 0.5.3
> **Última atualização:** 2026-05-11 (hybrid search BM25 + dense via Qdrant RRF)
...
│   │   └── bm25/              # Modelos BM25 serializados por coleção (JSON)
...
│   │   ├── rag.py              # build_rag_context() — entry point + hybrid search
│   │   ├── sparse.py           # BM25Vectorizer — Okapi BM25 custom (tokenizer, fit, transform, save/load)
...
3. **Hybrid search** (v0.5.3):
   a. Carrega modelo BM25 de `data/qdrant/bm25/{collection}.json` (cache por coleção)
   b. Tokeniza query (whitespace + accent-stripping + stop-word removal PT+EN)
   c. Gera sparse query vector via `BM25Vectorizer.transform()`
   d. `VectorStore.query()` com dense + sparse → Qdrant Prefetch + RRF (Reciprocal Rank Fusion)
   e. Fallback gracioso: se modelo BM25 não existe, usa dense-only (backward compatible)
...
6. Pesquisa multi-estratégia:
   a. Primária: **hybrid search** — dense vectors + BM25 sparse vectors com Qdrant Prefetch + RRF (notas + código). Fallback dense-only quando modelo BM25 não disponível
...
| Módulo            | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `base.py`         | `VectorStore` Protocol (`@runtime_checkable`) com 5 métodos + `QueryResult` dataclass + `create_store()` factory (Qdrant-only desde v0.5.2). `upsert_batch()` aceita `sparse_vectors` opcional; `query()` aceita `sparse_query` opcional (hybrid search). `chroma_store.py` e `chromadb` completamente removidos                                                                                                                                                                                       |
| `qdrant_store.py` | `QdrantVectorStore` — Qdrant (embedded ou server mode), 1024d cosine, SHA256 string→uint64 ID mapping, `meta.json` recovery automático. Suporta sparse vectors BM25 via `SparseVectorParams()` (named vector "bm25"). `_ensure_collection()` cria/atualiza coleções com `sparse_vectors_config`. `upsert_batch()` armazena sparse vectors junto com dense. `query()` usa Qdrant Prefetch + RRF (Reciprocal Rank Fusion) para hybrid retrieval quando `sparse_query` fornecido. Único backend suportado |

### Retrieval (`obsidian_rag/retrieval/`)

| Módulo             | Função                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rag.py`           | `build_rag_context()` — entry point. Usa `VectorStore` protocol via `_get_store()` singleton. Adaptive top_k (`_estimate_complexity`). **Hybrid search:** `_vector_search()` carrega modelo BM25 do disco, tokeniza query (com stop-word removal), gera sparse query vector e passa dense + sparse ao `store.query()`. `_get_sparse_query()` helper com cache por coleção. Fallback gracioso para dense-only quando não existe modelo BM25. Multi-estratégia: vector (hybrid) + keyword + title. Deduplicação, threshold dinâmico, relevance gate |
...
| `sparse.py`        | `BM25Vectorizer` — implementação custom Okapi BM25 (~150 linhas). `tokenize()`: whitespace + accent-stripping tokenizer. Parâmetros: k1=1.5, b=0.75 (standard Okapi BM25). Ciclo de vida fit/transform/save/load para produção de sparse vectors. Persistência JSON em `data/qdrant/bm25/{collection}.json`. Zero dependências externas                                                                                                                                                                                                           |
...
**Qdrant server mode (opcional):** O `docker-compose.yml` inclui um serviço `qdrant` (qdrant/qdrant:v1.13.2) nas portas 6333/6334, ativado via profile: `docker compose --profile qdrant up`. Para usar Qdrant em server mode (recomendado para >50k chunks), configurar `qdrant_url = "http://localhost:6333"` em `rag.toml`. Por defeito, o Qdrant corre em modo embedded (sem servidor externo).
...
**386 testes** (3 skipped) em 22 ficheiros, sem dependências externas (Ollama):
...
| `test_sparse.py`              | 20     | BM25Vectorizer: tokenizer (whitespace, accents, lowercase), fit (IDF, avg_dl), transform (sparse vector format), persistence (save/load JSON), custom params (k1, b), edge cases                                                                                                                      |
| `test_vector_store.py`        | 47     | VectorStore protocol: Qdrant (embedded). upsert, count, get_existing_ids, delete, query (incl. `filters=` metadata filtering), collection isolation, cosine similarity, factory `create_store()`. **Hybrid search (9 novos):** upsert com sparse vectors, hybrid query com RRF, fallback dense-only   |
...
| Limitação                         | Descrição                                                                                                                                                                                                                                                                                                                                         |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Cobertura de testes parcial**   | 386 testes (3 skipped, unit + integration) cobrindo chunking (markdown + code + tree-sitter), router, budget, API auth, CLI, segurança, performance, adaptive top_k, vault_sync, cross-platform, manifest, ingest pipeline, ResourceGovernor, VectorStore protocol, Dask engine, BM25 sparse vectors e hybrid search. Faltam e2e tests com Ollama |
...
| Aspeto                  | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Versão**              | 0.5.3 (hybrid search BM25 + dense, 2026-05-11)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| **Pipeline RAG**        | Completo e funcional: sync (4 backends) → chunking (Markdown headers + Python AST + tree-sitter multi-linguagem) → embeddings (dense + BM25 sparse) → Qdrant (único vector store) → **hybrid retrieval** (dense + sparse com RRF) multi-estratégia com adaptive top_k + query-time metadata filtering. **Hybrid search (v0.5.3):** pós-ingest, `_rebuild_bm25_index()` fita BM25Vectorizer em todos os documentos da coleção e persiste modelo JSON em `data/qdrant/bm25/`. At query time, modelo carregado uma vez e cacheado por coleção; sparse query vector gerado e passado ao Qdrant via Prefetch + RRF. Fallback gracioso para dense-only quando modelo BM25 não existe. Zero novas dependências, zero VRAM. **Notas e repos processados via bounded ingest pipeline** (v1.1): 4 estágios (scanner → parser pool → embedding batcher → writer) conectados por queues bounded com backpressure. `IngestManifest` SQLite fornece skip incremental (mtime/size/SHA256) para ambos. **Parser pool via factory `create_parser_pool()`** (Phase 5): `ProcessPoolExecutor` (default, `engine="local"`) ou `DaskParserPool` distribuído (`engine="dask"`, requer `pip install obsidian-rag[dask]`). `ResourceGovernor` (Phase 2) monitoriza recursos continuamente via daemon thread. `os.nice(10)` para não bloquear o desktop. Proteção de recursos ao nível do OS via `rag-cgroup.sh` (opcional). **VectorStore Protocol** (Phase 3): abstração `VectorStore` com factory `create_store()` (Qdrant-only desde v0.5.2). `upsert_batch()` aceita `sparse_vectors` opcional; `query()` aceita `sparse_query` e `filters: dict` para metadata filtering e hybrid search. ChromaDB completamente removido em v0.5.2 |
...
| **Documentação**        | Este ficheiro + `IMPROVEMENTS_AND_RISKS.md` + roadmap v1.1 (Fase 20 — 13 tarefas de escalabilidade, #182 hybrid search concluída)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| **Testes**              | 386 testes (3 skipped) em 22 ficheiros com pytest. Todos passam sem deps externas. Cobertura: 61%. Hotfixes v0.5.1 + GPU optimization + query filtering + adaptive top_k + hybrid search BM25 (2026-05-11) validados                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
